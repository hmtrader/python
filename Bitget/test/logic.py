...
def calculate_atr(df, period=14):
    ...
    #     return print(f"Lỗi ở calculate_atr: {e}")
...
def analyze_trend(df):
    ...
    try:
        ...
        return print(f"analyze_trend: {e}")    
    
    if pd.isna(ema9) or pd.isna(ema21) or pd.isna(rsi) or len(df) < 21:
...
def is_bullish_engulfing(df, n=CANDLES, min_count=2):
    ...
    return len(positions) >= min_count
